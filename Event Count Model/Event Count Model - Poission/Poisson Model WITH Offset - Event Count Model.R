#################### Event Count Model - Poission WITH Offset
# Read in data
...
names(bills)
# Poisson Model of Bills Receiving Action with Offset (Bills Proposed)

mydata <- read.table("Bills2.txt", header=TRUE, sep="\t")
...
poiss.action2 <- glm(action ~ seniority + 
                       majority + 
                       chair + 
                       subchair + 
                       female + 
                       ...
                       distance, 
                     family = poisson, 
                     offset = log(bills), #Take into account, how many bills a MC Proposes. Take into account how many bills a MC proposes.
                     data = bills2)

sum.poiss.action2 <- summary(poiss.action2); sum.poiss.action2


# Get rid of annoying floating coefficients

coeffs2 <- round(poiss.action2$coef, 3); coeffs2


# Incident rate ratios

irrs2 <- exp(coef(poiss.action2)); irrs2


# Confidence intervals for betas, irrs

ci.beta2 <- confint(poiss.action2); ci.beta2

ci.irr2 <- exp(confint.default(poiss.action2)); ci.irr2
