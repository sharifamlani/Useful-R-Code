...
#################### KNN ###########################
#http://rpubs.com/harshaash/KNN_imputation

library(caret)
library(RANN)

set.seed(1985)
preProcValues  <- preProcess(Ideology, method=c("knnImpute"),
                     k = 2,
                     knnSummary = mean)
dat.imputed <- predict(preProcValues, Ideology)

procNames <- data.frame(col = names(preProcValues$mean), mean = preProcValues$mean, sd = preProcValues$std)
for(i in procNames$col){
  dat.imputed[i] <- dat.imputed[i]*preProcValues$std[i]+preProcValues$mean[i] 
}
